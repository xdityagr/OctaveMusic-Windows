...
# FIXME take screenshots in interval when window is not moving.
# FIXME If you import pyqt5, an error comes ; that QPIxmap only supports QImage not ImageQt.
# TODO Optimize Custom QCFX Blur, it crashes very often

# QC FX Settings & Modes Handling class
class QCFxInitializer:
    def __init__(self):
...
class QCFx_Blur:
    def __init__(self, parent, overlay_parent=None, overlay=None, mode=None):
        ...
        if overlay==None :#and self.overlay_parent != None
    ...
    def generateParentBackground(self):
        ...
        self.blurLayer.setGeometry(self.parent.rect())  # Update blur layer geometry
    ...
    @Slot()
    def on_parent_resized(self, size):
        new_size=self.parent.size()
        print(f"Parent widget resized to: {new_size.width()}x{new_size.height()}")
        self.generateParentBackground()
    ...
    def endBlur(self):
        # # Stop fixed update timer
        # if hasattr(self, 'RFUpdate') and self.RFUpdate.isActive():
        #     self.RFUpdate.stop()
        
        # # Stop movement timer
        # if hasattr(self, 'movement_timer') and self.movement_timer.isActive():
        #     self.movement_timer.stop()

        # # Stop stopped timer
        # if hasattr(self, 'stopped_timer') and self.stopped_timer.isActive():
        #     self.stopped_timer.stop()

        # # Stop any background capture threads
        # if hasattr(self, 'monitor_thread') and self.monitor_thread.is_alive():
        #     self.monitor_thread.join(timeout=1)  # Wait for thread to terminate
        # if hasattr(self, 'check_windows_thread') and self.check_windows_thread.is_alive():
        #     self.check_windows_thread.join(timeout=1)  # Wait for thread to terminate
        # if hasattr(self, 'wallpaper_thread') and self.wallpaper_thread.is_alive():
        #     self.wallpaper_thread.join(timeout=1)  # Wait for thread to terminate

        # # Cancel any ongoing image processing threads
        # if self.processing_thread and self.processing_thread.is_alive():
        #     self.processing_thread.join(timeout=1)  # Wait for thread to terminate

        # Clean up
        if self.blurLayer:
            self.blurLayer.clear()  # Clear the pixmap to release resources

        # Reset any remaining references or settings
        # self.parent = None
        # self.overlay_parent = None
        self.blurLayer = None
        self.cached_image = None
        self.cropped_image_qimage = None
    ...
    def endAll(self):
        ...
        self.endBlur_thread = threading.Thread(target=target)
        self.endBlur_thread.start()
        self.endBlur_thread.join() 
