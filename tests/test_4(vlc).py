...
results = []
try:
    result1 = loop.run_until_complete(ocf.fetchQuery("uptown funk mark ronson", results=1))
    result2 = loop.run_until_complete(ocf.fetchQuery("true colors the weeknd", results=1))
finally:
    loop.close()
...
class MASS:
    ...
    def _player_worker(self, stream_url, control_queue, volume, update_callback):
        ...
        try:
            ...
            # Start a separate thread to track the position
            def position_tracker():
                while True:
                    if self.is_playing:
                        try:
                            # Attempt to get the current playback position
                            pts = player.get_pts()  # Try to get the playback time
                            if pts is not None:
                                current_position_ms = int(pts * 1000)
