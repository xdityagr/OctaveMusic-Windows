...
class OctaveFetch:
    ...
    def __generate_client_file(self,  force_new=False, force_verify=False):
    ...
    def _grape_fetch_sanitizedata(self, contents, query_correction_info, audio_client):
    ...
    def _grape_fetch_search_content(self, query, search_client, continuation_token=None):
    ...
    def grape_fetch(self, query, limit):
        ...
        if initial_length < limit:
            ...
            while start_len < limit:
                ...
                continuation_contents, curr_continuation_token= self._grape_fetch_search_content(query, search_client, curr_continuation_token)
            ...
            sorted_data = self._grape_fetch_sanitizedata(net_content, query_correction_info, audio_client)
        else:
            print(f'Found {len(contents)} results, fetching & filtering now ...')
            if initial_length > limit:
                contents = contents[:limit]
            sorted_data = self._grape_fetch_sanitizedata(contents, query_correction_info, audio_client)
