...
from OctaveEngine.definitions import *
from OctaveEngine.MediaControls_VLC import VLCService
from . import *
...
class OctaveQueue:
    ...
    def addToQueue(self, LocalMedia):
        if self.current_queue is not None:
            ...
            warn("Octave Engine", "QueueManager:addToQueue", f"Queue is empty, Created new.")  
...
class OctaveAdvMediaController: 
    def __init__(self, parent, debug=False,callback=None, backend= OctaveAudioBackend.DEFUALT):
        self.backend = backend
        self.usingVLC = False
        if self.backend ==  OctaveAudioBackend.VLC:
            self.usingVLC = True
            self.MediaController2 = VLCService(debug=debug, callback=callback)

        # if self.backend == OctaveAudioBackend.DEFUALT:
    ...
    def setMedia(self, MediaObject):
        ...
        if self.MediaObject.isStreamable: 
            ...
            #mc2
            if self.usingVLC:
                self.MediaController2.set_media(self.MediaObject)
            ...
            if elapsed_time >= 0.01:
                with self.lock:
                    if self.isMediaStreamable: 
                        ...
                        print(f"Current playback position: {current_position} seconds")
                    ...
                    if self.position_callback is not None:
                        ...
                        warn('no position callback found')
    ...
    def set_position_callback(self, callback):
        ...
        # mc2
        if self.usingVLC:
            self.MediaController2.set_callback(callback)
    ...
    def seek(self, seconds): 
        ...
        if self.isMediaStreamable:
            ...
            if self.usingVLC:
                self.MediaController2.seek(seconds)
            else:
            ...
            if self.position_callback is not None:
                print(f'self.curr pos : {self.current_position}')
    ...
    def setVolume(self, VolumeLevel): 
        if 0.0 <= VolumeLevel <= 1.0:
            ...
            if self.isMediaStreamable:
                ...
                if self.usingVLC:
                    self.MediaController2.set_volume(int(VolumeLevel*100))
                else:
                    if self.stream_player_obj is not None:
...
# class OctaveFetch:
#     async def fetchQuery(self, query, results=6, codec=OctaveStreamSupportedCodec.m4a, quality=OctaveStreamAudioQuality.WORSE):
